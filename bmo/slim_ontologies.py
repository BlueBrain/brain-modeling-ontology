...
def create_slim_ontology_graph(
    original_graph: Graph,
    keep_attributes: List[term.URIRef] = SLIM_GRAPH_PREDICATES,
    keep_types: List[term.URIRef] = SLIM_GRAPH_TYPES,
) -> Graph:
...
def create_slim_classes(
    original_classes: List[Dict], keep_attributes: List[str] = SLIM_CLASS_ATTRIBUTES
) -> List[Dict]:
    """Remove any property from the classes that is not in the provided list of attributes.
    ...
    """
    new_classes = [
        {k: v for k, v in oclass.items() if k in keep_attributes}
...
def get_slim_ontology_id(original_id: term.URIRef) -> term.URIRef:
    ...
    base_uri = (
        str(original_id)
        if not str(original_id).endswith("/")
        else str(original_id)[:-1]
    )
